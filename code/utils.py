...
class Indexer(object):
    ...
    # Adds the object to the index if it isn't present, always returns a
    # nonnegative index
...
# Relatively inefficient (dictionary-backed); useful for sparse encoding of
# things like gradients, but shouldn't be used for dense things like weight
# vectors (instead use an Indexer over the objects and use a numpy array to
# store the values)
class Counter(object):
    ...
    def __repr__(self):
        return str([str(key) + ": " + str(self.get_count(key)) \
    ...
    def argmax(self):
        ...
        for key in self.counter.keys():
            if best_key is None \
                    or self.get_count(key) > self.get_count(best_key):
                best_key = key
        return best_key


# Beam data structure. Maintains a list of scored elements like a Counter, but
# only keeps the top n elements after every insertion operation. Insertion is
# O(n) (list is maintained in sorted order), access is O(1). Still fast enough
# for practical purposes for small beams.
class Beam(object):
    ...
    # Adds the element to the beam with the given score if the beam has room or
    # if the score is better than the score of the worst element currently on
    # the beam
    def add(self, elt, score):
        ...
        # Find the insertion point with binary search
            ...
            # We're searching for the index of the first element with score
            # less than score
            while lb < ub:
                m = (lb + ub) // 2
                # Check > because the list is sorted in descending order
                if self.scores[m] > score:
                    # Put the lower bound ahead of m because all elements
                    # before this are greater
                    lb = m + 1
                else:
                    # m could still be the insertion point
                    ub = m
            # lb and ub should be equal and indicate the index of the first
            # element with score less than score.
...
# Computes the dot product over a list of features (i.e., a sparse feature
# vector) and a weight vector (numpy array)
